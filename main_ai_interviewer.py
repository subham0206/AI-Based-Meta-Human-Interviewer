...
def speak_text(questions):
    """
    Function to convert a list of questions (or any text) to speech.
    
    Args:
    questions (list): A list of strings (questions or text) to be spoken.
    ...
    # Set properties (optional adjustments)
    engine.setProperty('rate', 150)  # Speed (higher is faster)
    engine.setProperty('volume', 1)  # Volume (range from 0.0 to 1.0)

    # Speak each question
    for question in questions:
        engine.say(question)  # Add each question to the speech queue
...
def main():
    ...
    if st.button("Analyze Resumes") and jd_text and uploaded_files:
        ...
        if all_results:
            ...
            questions = [
            "How do you utilize Python and SQL in your data science projects?",
            "How do you ensure data quality and accuracy in your predictive models?",
            "Can you explain how you would approach building a machine learning model for demand-based rent pricing?",
            "How do you prioritize tasks when working on multiple projects with tight deadlines?",
            "What was your role in the AB testing initiatives at Bright Analytics, and how did it improve marketing ROI?"
            ]
